...
class Chart(QWidget):
    ...
    def __init__(self, secadora, fecha, temperaturas, temp_min, temp_max):
        ...
        if(self.temp_min and self.temp_max):
            ...
            series3 << QPointF(float(0), float(self.temp_min)) << QPointF(
                float(200), float(self.temp_min))
            series4 << QPointF(float(0), float(self.temp_max)) << QPointF(
                float(200), float(self.temp_max))
        ...
        for item in self.data:
            ...
            date = sp1[0].split("-")
            hour = sp1[1].split(":")
            itemDate = datetime.datetime(
                int(date[0]), int(date[1]), int(date[2]))

            if(itemDate == datetime.datetime(y, m, d) and item['secadora'] == self.secadora):
                series.append(QDateTime.fromString(
                    item['fecha'], "yyyy-MM-dd hh:mm:ss").toMSecsSinceEpoch(), float(item['temperatura']))
                series2.append(QDateTime.fromString(
                    item['fecha'], "yyyy-MM-dd hh:mm:ss").toMSecsSinceEpoch(), float(item['temperatura2']))
        ...
        chart.axisY(series).setRange(
            min(self.temperaturas)-5, max(self.temperaturas)+5)
